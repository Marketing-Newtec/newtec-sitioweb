...
const FeatureCard: React.FC<{ title: string; desc: string; icon: string; delay: number }> = ({ title, desc, icon, delay }) => (
  <motion.div 
    ...
    viewport={{ once: true }}
    className="glass-card p-8 md:p-12 rounded-[2rem] md:rounded-[2.5rem] transition-all hover:-translate-y-4 group hover:border-white/30 hover:bg-white/5 relative overflow-hidden"
  >
...
export const Features: React.FC = () => {
  return (
    <section id="tecnología" className="py-24 md:py-40 px-6 relative">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-10 md:gap-12 mb-20 md:mb-32">
          <motion.div 
            ...
            className="max-w-4xl"
          >
            {/* Título: Ajustado con vw para móviles y escala moderada para laptops de 1440px */}
            ...
            <div className="h-1.5 md:h-2 w-32 md:w-48 bg-gradient-to-r from-white to-transparent rounded-full"></div>
          </motion.div>

          {/* Bajada: Ajuste de borde y tamaño para mayor legibilidad en móviles */}
          <motion.p 
            initial={{ opacity: 0, x: 30 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className="text-lg md:text-xl text-purple-100/70 max-w-md font-light border-l-2 border-purple-400/30 pl-6 md:pl-8 leading-relaxed"
          >
