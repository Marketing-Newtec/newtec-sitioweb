...
export const Header: React.FC<HeaderProps> = ({ scrolled }) => {
  ...
  const LanguageSelector = ({ mobile = false }: { mobile?: boolean }) => (
    <div className={`flex items-center gap-4 ${mobile ? 'mt-12 justify-center' : 'ml-8 border-l border-white/20 pl-8'}`}>
      ...
      {/* Botón ES - Vuelve a la web principal en español */}
      <button 
        onClick={() => window.location.href = 'https://lavenderblush-snake-373826.hostingersite.com/'}
      ...
      {/* Botón EN - Apunta directo al dominio de Vercel para evitar el error 404 */}
      <button 
        onClick={() => window.location.href = 'https://en-newtec.vercel.app'}
        className="flex items-center gap-2 group transition-all"
      >
        <img 
